...
$fileEntention=strtolower((pathinfo($filename,PATHINFO_EXTENSION)));

if(in_array($fileEntention,$allowedExtensions)){

    if(move_uploaded_file($file['tmp_name'],$uploadDir.$filename)){
     $messages[]=['text'=>"Uploaded!!",'color'=>"text-green-500"];
     $submitted = true;
    } else {
        $messages[]=['text'=>"ERROR IMAGE UPLOADING!!",'color'=>"text-red-500"];
        $submitted = false;
    }

} else{
    $messages[]=['text'=>"INVALID FILE TYPE!!",'color'=>"text-blue-500"];
    $submitted = false;
}
